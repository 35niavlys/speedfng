...
void IGameController::StartRound()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		{
			/*CCharacter *pOwnerChar = GameServer()->GetPlayerChar(i); // maybe bug :(
			if(pOwnerChar && pOwnerChar->Core()->m_Protected)
			    pOwnerChar->Core()->m_Protected = false;*/
			
			/*GoJE GrEEN !*/
			GameServer()->m_apPlayers[i]->m_HammerFreeze = false;
			GameServer()->m_apPlayers[i]->m_JetPack = false;
			GameServer()->m_apPlayers[i]->m_SpeedRunner = false;
			GameServer()->m_apPlayers[i]->m_RifleSpread = false;
			GameServer()->m_apPlayers[i]->m_Invisible = false;
			GameServer()->m_apPlayers[i]->m_RifleSwap = false;
			GameServer()->m_apPlayers[i]->m_Pauseable = false;
			GameServer()->m_apPlayers[i]->m_TeamProtect = false;
			GameServer()->m_apPlayers[i]->m_GrenadeLauncher = false;
...
void IGameController::MakeOpenFng(bool Option)
{
	m_pOpenFng = Option;
}

bool IGameController::IsOpenFng()
{
	return m_pOpenFng;
}
