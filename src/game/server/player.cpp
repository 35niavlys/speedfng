	...
	if(!GameServer()->m_World.m_Paused)
	{
		...
		if(m_pCharacter)
		{
			if(m_pCharacter->IsAlive())
			{
				...
				{
					if((!m_pCharacter->GetWeaponGot(WEAPON_NINJA) || m_pCharacter->GetFreezeTicks()) && m_pCharacter->IsGrounded() && m_pCharacter->m_Pos == m_pCharacter->m_PrevPos)
						ProcessPause();
				}
...
void CPlayer::PostTick()
{
	...
	/*if(GetBot){
	((CServer *)Server())->m_ServerBan.BanAddr(((CServer *)Server())->m_NetServer.ClientAddr(m_ClientID), g_Config.m_SvRconBantime*60, "[Bot]: Detected !");
	    GetBot = false;}*/
...
void CPlayer::ProcessPause()
{
	...
	if(m_Paused >= PAUSED_PAUSED)
	{
		if(!m_pCharacter->IsPaused())
		{
			...
			GameServer()->CreateSound(m_pCharacter->m_Pos, SOUND_PLAYER_DIE, CmaskOne(m_ClientID));
...
{
 	if(GameServer()->m_World.m_Paused || !m_pCharacter || m_Team == TEAM_SPECTATORS || m_pCharacter->GetFreezeTicks() || m_Paused != PAUSED_NONE)
 	{
 		m_CampTick = -1;
 		m_SentCampMsg = false;
 		return 0;
