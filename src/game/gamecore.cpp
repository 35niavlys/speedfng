...
void CCharacterCore::Tick(bool UseInput)
{
	...
	else if(m_HookState == HOOK_FLYING)
	{
		...
		if(this->m_Hooking && m_pWorld && m_pWorld->m_Tuning.m_PlayerHooking)
		{
			float Distance = 0.0f;
	...
	if(m_pWorld)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if(m_Hooking && m_HookedPlayer == i && m_pWorld->m_Tuning.m_PlayerHooking && !pCharCore->m_Protected && !pCharCore->m_ProtectedBy || (m_Hooking && m_HookedPlayer == i && m_pWorld->m_Tuning.m_PlayerHooking && pCharCore->m_Protected && pCharCore->m_Frozen <= 0))
			{
	...
	if(m_pWorld && m_pWorld->m_Tuning.m_PlayerCollision && this->m_Collision)
	{
		...
		for(int i = 0; i < End; i++)
		{
			...
			for(int p = 0; p < MAX_CLIENTS; p++)
			{
				...
				if(!pCharCore || pCharCore == this || !pCharCore->m_Collision)
					continue;
